...
export default class ViewRenderer {
  ...
  private calculateWallHeightSimple(
    ...
    if (V2XScreen >= this.renderer3d.width - 1) {
  ...
  private partialSeg(
    ...
    const halfFOV = this.player.FOV.getValue() / 2;
    ...
    if (IsLeftSide) {
      AngleVToFOV = V1Angle.subtract(
        new Angle(this.player.getAngle().getValue() + halfFOV)
      );
    } else {
      AngleVToFOV = new Angle(this.player.getAngle().getValue() - halfFOV).subtract(
        V2Angle
      );
    }
